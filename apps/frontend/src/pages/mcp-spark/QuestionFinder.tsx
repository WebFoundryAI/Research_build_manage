        ...
        <>
          ...
          <div className="flex flex-wrap gap-2">
            <button
              ...
              className={`px-3 py-1.5 text-sm rounded-lg transition-colors ${activeCategory === "all" ? "bg-blue-600 text-white" : "border border-slate-700 text-slate-300 hover:bg-slate-800"}`}
            >
            ...
            {result.categories?.map((cat) => (
              <button
                ...
                className={`px-3 py-1.5 text-sm rounded-lg transition-colors ${activeCategory === cat.category ? "bg-blue-600 text-white" : "border border-slate-700 text-slate-300 hover:bg-slate-800"}`}
              >
            ...
            <div className="p-6">
              <div className="flex gap-2 mb-4 border-b border-slate-700">
                <button
                  ...
                  className={`px-4 py-2 text-sm flex items-center gap-1 border-b-2 transition-colors ${activeTab === "questions" ? "border-blue-500 text-blue-400" : "border-transparent text-slate-400 hover:text-slate-300"}`}
                >
                ...
                <button
                  ...
                  className={`px-4 py-2 text-sm flex items-center gap-1 border-b-2 transition-colors ${activeTab === "ideas" ? "border-blue-500 text-blue-400" : "border-transparent text-slate-400 hover:text-slate-300"}`}
                >
                ...
                {activeTab === "questions" && (
                  <div className="space-y-3">
                    {filteredQuestions.map((q, index) => (
                      <div
                        key={index}
                        className="flex items-start justify-between gap-3 p-3 rounded-lg bg-slate-800/50 hover:bg-slate-800 transition-colors group"
                      >
                        <div className="flex-1">
                          <div className="flex items-center gap-2 mb-1">
                            <span className="text-sm">{getCategoryIcon(q.category)}</span>
                            <span className="px-2 py-0.5 text-xs border border-slate-600 rounded text-slate-400 capitalize">{q.category}</span>
                            {q.popularity && (
                        ...
                        <button
                          ...
                          className="p-1.5 opacity-0 group-hover:opacity-100 text-slate-400 hover:text-white transition-all"
                        >
                          {copiedIndex === index ? (
                  ...
                  <div className="space-y-3">
                    {result.contentIdeas?.map((idea, index) => (
                      <div key={index} className="flex items-start gap-3 p-4 rounded-lg bg-blue-500/10 border border-blue-500/20">
                        ...
                        <p className="text-sm font-medium text-slate-300">{idea}</p>
                      </div>
                    ))}

...
export default function Monitoring() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case "checking":
        return (
          <span className="px-2 py-0.5 text-xs bg-slate-700 text-slate-400 rounded flex items-center gap-1">
            <RefreshCw className="h-3 w-3 animate-spin" />
            Checking
          </span>
        );
      default:
        return <span className="px-2 py-0.5 text-xs bg-slate-700 text-slate-400 rounded">{status}</span>;
    }
  };
        ...
        <div className="p-6">
          <div className="flex gap-4">
            ...
            <button
              ...
              className="px-4 py-2 border border-slate-700 text-slate-300 hover:bg-slate-800 disabled:opacity-50 rounded-lg flex items-center gap-2 transition-colors"
            >
        ...
        <div className="p-6">
          {domains.length > 0 ? (
            <table className="w-full">
              ...
              <tbody>
                {domains.map((domain) => (
                  <tr key={domain.id} className="border-b border-slate-800">
                    <td className="p-3">
                    ...
                    <td className="p-3 text-center">
                      <div className="flex items-center justify-center gap-2">
                        ...
                        <span className="text-sm text-slate-300">{domain.uptime}%</span>
                      </div>
                    </td>
                    <td className="p-3 text-center">
                      {domain.rankChange !== 0 && (
                        <div className={`flex items-center justify-center gap-1 ${domain.rankChange > 0 ? "text-green-400" : "text-red-400"}`}>
                    ...
                    <td className="p-3 text-right text-slate-300">{domain.keywords}</td>
                    <td className="p-3 text-sm text-slate-500">{formatDate(domain.lastCheck)}</td>
                    <td className="p-3 text-center">
                      <button onClick={() => removeDomain(domain.id)} className="p-2 text-red-400 hover:text-red-300 hover:bg-slate-800 rounded transition-colors">

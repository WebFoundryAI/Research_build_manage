        ...
        <div className="p-6 space-y-4">
          <div className="space-y-2">
            <div className="flex justify-between items-center">
              <label className="text-sm font-medium text-slate-300">Research Question</label>
            ...
            <textarea
              ...
              className="w-full px-4 py-3 bg-slate-800 border border-slate-700 rounded-lg text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500 resize-y min-h-[100px]"
            />
          ...
          <div className="flex flex-col md:flex-row gap-4">
            <div className="flex-1 space-y-2">
              <label className="text-sm font-medium text-slate-300">Research Depth</label>
              <div className="flex gap-2 flex-wrap">
                {researchModes.map((m) => (
                  <button
                    ...
                    disabled={isLoading}
                    className={`px-3 py-1.5 text-sm rounded-lg flex items-center gap-1 transition-colors ${mode === m.id ? "bg-blue-600 text-white" : "border border-slate-700 text-slate-300 hover:bg-slate-800"}`}
                  >
            ...
            <div className="w-full md:w-48 space-y-2">
              <label className="text-sm font-medium text-slate-300">Max Sources</label>
              <select
                value={maxSources}
        ...
        <div className="rounded-xl border border-slate-800 bg-slate-900/50 overflow-hidden">
          <div className="p-4 border-b border-slate-800 flex items-start justify-between">
            ...
            <div className="flex gap-2">
              <button onClick={handleCopy} className="px-3 py-1.5 text-sm border border-slate-700 rounded-lg text-slate-300 hover:bg-slate-800 flex items-center gap-1 transition-colors">
                    ...
                    <div key={index} className="rounded-lg border border-slate-700 bg-slate-800/50 p-4">
                      <div className="flex items-start justify-between gap-4">
                        <div className="flex-1 min-w-0">
                          ...
                          <a href={source.url} target="_blank" rel="noopener noreferrer" className="text-xs text-blue-400 hover:underline flex items-center gap-1 mt-1">
                            {source.url.slice(0, 60)}...
                            <ExternalLink className="h-3 w-3" />
                          </a>
                          <p className="text-sm text-slate-400 mt-2">{source.snippet}</p>
                        </div>
                        <span className="px-2 py-0.5 text-xs bg-slate-700 rounded text-slate-400">{index + 1}</span>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div className="flex flex-wrap justify-center gap-2 mt-4">
              {[
                ...
                <button
                  ...
                  className="px-3 py-2 text-xs border border-slate-700 rounded-lg text-slate-400 hover:bg-slate-800 text-left whitespace-normal transition-colors"
                >
